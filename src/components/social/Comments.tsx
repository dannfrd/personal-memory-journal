...
export function Comments({ postId }: { postId: string }) {
  ...
  return (
    <div className="w-full max-w-2xl">
      <h3 className="mb-8 font-serif text-3xl tracking-tight opacity-90">Reflections ({comments.length})</h3>
          ...
          {mainComments.map((comment) => (
            <div key={comment.id} className="group">
                  ...
                  {getReplies(comment.id).map(reply => (
                    <div key={reply.id} className="group/reply">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className="font-bold tracking-[0.2em] uppercase text-[10px] opacity-80">{reply.username}</span>
                        <span className="h-1 w-1 rounded-full bg-current opacity-30"></span>
                        <span className="text-[10px] uppercase tracking-[0.2em] opacity-40">{formatDate(reply.created_at)}</span>
                      </div>
              ...
              {replyToId === comment.id && (
                <form onSubmit={(e) => handleReplySubmit(e, comment.id)} className="mt-6 pl-6 sm:pl-10 animate-in fade-in slide-in-from-top-2 flex flex-col gap-4">
                  <div className="flex items-center gap-2 mb-1">
                    <CornerDownRight className="w-3 h-3 opacity-40" />
                    <span className="text-[10px] font-bold uppercase tracking-[0.2em] opacity-60">Replying to {comment.username}</span>
                  </div>
                  <input 
                    type="text" 
                  ...
                  <button 
                    type="submit" 
                    disabled={submitting}
                    className="self-start rounded-full border border-current px-6 py-2.5 text-[10px] font-bold uppercase tracking-[0.2em] transition-all hover:bg-current hover:text-white hover:bg-opacity-10 disabled:opacity-50 dark:hover:text-black dark:hover:bg-opacity-100"
                  >
      ...
      {/* Main Comment Form */}
      <form onSubmit={handleSubmit} className="flex flex-col gap-6 pt-10 border-t border-current border-opacity-10">
        <h4 className="text-xs font-bold uppercase tracking-[0.2em] opacity-60 mb-2">Leave a new reflection</h4>
        <input 
          type="text" 
          placeholder="Your Name / Initial" 
          value={username}
